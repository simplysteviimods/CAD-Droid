...
# NEW: Phase header system using exact same logic as intro cards
# Draw a phase header for major installation steps
draw_phase_header(){
  local text="${1:-}"
  
  # Split the text on first newline to get title and subtitle (like intro cards)
  local title="${text%%
...
run_with_progress(){
  ...
  while kill -0 "$pid" 2>/dev/null; do
    ...
    # Use echo instead of printf to avoid escaping issues
    printf "\r"
    echo -n "${sym} ${message} (${pct}%)"
...
main_execution "$@"
  ...
  # Display title if provided (exact same logic as intro cards)
  if [ -n "$title" ]; then
    printf "%b%s%b\n" "$mid" "$(center_text "$title")" '\033[0m'
  fi
  
  # Display subtitle if provided (exact same logic as intro cards)
  if [ -n "$subtitle" ]; then
    printf "%b%s%b\n" "$mid" "$(center_text "$subtitle")" '\033[0m'
  fi
