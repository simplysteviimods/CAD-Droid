...
# Prefer F-Droid? Set PREFER_FDROID=1 to use F-Droid store over GitHub for APKs (F-Droid is default)
...
# NEW: Simplified phase header system - uses same logic as intro cards
...
# NEW: Use Termux file picker to select APK directory with improved UX and fallback
select_apk_directory(){
  ...
  if [ "$NON_INTERACTIVE" != "1" ]; then
    ...
    # Use termux-storage-get with timeout to avoid hanging
    local temp_file="$TMPDIR/selected_path.txt"
    
    info "Opening file picker (30 second timeout)..."
    if timeout 30 termux-storage-get "$temp_file" 2>/dev/null; then
...
# NEW: Enhanced Termux add-on fetch that preserves original APK names and prioritizes F-Droid
...
create_widget_shortcuts(){
  ...
  # Create Portrait Mode Desktop shortcut
  cat > "$shortcuts_dir/Desktop Portrait Mode.sh" <<PORTRAIT_WIDGET_EOF
#!/data/data/com.termux/files/usr/bin/bash
set -e
echo "Starting Desktop Environment - Portrait Mode"

# Get setup configuration
DISTRO="${DISTRO:-ubuntu}"
UBUNTU_USERNAME="${UBUNTU_USERNAME:-caduser}"

# Start container and SSH in background
{
  echo "Starting Linux container as root and SSH daemon..."
  # First subshell: Login as ROOT to start SSH daemon
  proot-distro login "\$DISTRO" --user root -- bash -c '
    echo "Starting SSH daemon as root..."
    /usr/sbin/sshd || /usr/sbin/sshd -D &
    sleep 3
  ...
  # Second subshell: Connect via SSH as REGULAR USER and launch desktop
  echo "Connecting via SSH as \$UBUNTU_USERNAME and launching desktop..."
  ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 cad-container bash -c '
    echo "Connected as regular user, launching XFCE..."
    # Launch XFCE desktop session
    export DISPLAY=:0
    if ! pgrep -f xfce4-session >/dev/null 2>&1; then
    ...
    echo "XFCE desktop session started as \$USER"
  ' &
  
  sleep 3
  
  # Launch Termux-X11 in portrait mode (runs in Termux, not container)
  echo "Starting Termux-X11 in portrait mode..."
  if ! pgrep -f termux-x11 >/dev/null 2>&1; then
...
echo "Desktop Portrait Mode starting in background..."
echo "Root subshell: SSH daemon"
echo "User subshell: \$UBUNTU_USERNAME desktop session"
echo "Desktop will be ready in a few seconds"
PORTRAIT_WIDGET_EOF

  # Create Landscape Mode Desktop with Sunshine shortcut
  cat > "$shortcuts_dir/Desktop Landscape Mode.sh" <<LANDSCAPE_WIDGET_EOF
#!/data/data/com.termux/files/usr/bin/bash
set -e
echo "Starting Desktop Environment - Landscape Mode with Streaming"
...
# Start container and SSH in background
{
  ...
  # Second subshell: Connect via SSH as REGULAR USER and launch desktop with Sunshine
  echo "Connecting via SSH as \$UBUNTU_USERNAME and launching desktop with streaming..."
  ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 cad-container bash -c '
    echo "Connected as regular user, launching XFCE with Sunshine..."
    # Launch XFCE desktop session
    export DISPLAY=:0
    if ! pgrep -f xfce4-session >/dev/null 2>&1; then
      DISPLAY=:0 xfce4-session >/dev/null 2>&1 &
    ...
    # Start Sunshine streaming service if available (as regular user)
    if command -v sunshine >/dev/null 2>&1; then
      sunshine >/tmp/sunshine.log 2>&1 &
      echo "Sunshine streaming service started as \$USER"
    fi
    
    echo "XFCE desktop with streaming started as \$USER"
  ' &
  
  sleep 3
  
  # Stop existing X11 server for orientation change
  pkill -f termux-x11 2>/dev/null || true
  sleep 1
  
  # Launch Termux-X11 in landscape mode (runs in Termux, not container)
  echo "Starting Termux-X11 in landscape mode..."
  termux-x11 :0 -geometry 1920x1080 >/dev/null 2>&1 &
...
echo "Desktop Landscape Mode with Streaming starting in background..."
echo "Root subshell: SSH daemon"
echo "User subshell: \$UBUNTU_USERNAME desktop + Sunshine"
echo "Desktop will be ready in a few seconds"
echo "Sunshine streaming will be available for remote access"
LANDSCAPE_WIDGET_EOF

  # Substitute actual values into the shortcuts
  local actual_distro="${DISTRO:-ubuntu}"
  local actual_username="${UBUNTU_USERNAME:-caduser}"
  
  # Replace placeholder values with actual configuration
  sed -i "s/DISTRO=\"\${DISTRO:-ubuntu}\"/DISTRO=\"$actual_distro\"/" "$shortcuts_dir"/*.sh 2>/dev/null || true
  sed -i "s/UBUNTU_USERNAME=\"\${UBUNTU_USERNAME:-caduser}\"/UBUNTU_USERNAME=\"$actual_username\"/" "$shortcuts_dir"/*.sh 2>/dev/null || true

  # Make all shortcuts executable
  chmod +x "$shortcuts_dir"/*.sh 2>/dev/null || true
  ok "Desktop widget shortcuts created - Portrait and Landscape modes with proper user separation"
  info "Root subshell: Starts SSH daemon"
  info "User subshell: $actual_username runs desktop session"
}
...
step_storage(){
  ...
  # Set proper permissions on .termux directory
...
# Step 8: Install core productivity packages with safe array iteration and arithmetic
step_coreinst(){
...
# NEW: Step 9: Enhanced APK Installation with detailed instructions and F-Droid priority
step_apk(){
  ...
  # Show detailed installation instructions BEFORE starting downloads
  echo ""
  draw_card "IMPORTANT: APK Installation Instructions" "Please read carefully before proceeding"
  echo ""
  
  info "Essential APK Installation Steps:"
  echo ""
  pecho "$PASTEL_YELLOW" "1. DOWNLOAD PHASE (happening now):"
  info "   - APK files will be downloaded to your selected folder"
  info "   - Wait for all downloads to complete"
  echo ""
  pecho "$PASTEL_YELLOW" "2. PERMISSION SETUP (required for proper function):"
  info "   After installing each APK, you MUST configure permissions:"
  echo ""
  pecho "$PASTEL_PINK" "   For Termux:API & Termux:GUI:"
  info "   → Settings > Apps > [App Name] > Permissions"
  info "   → Enable ALL requested permissions"
  echo ""
  pecho "$PASTEL_PINK" "   For Termux:X11:"
  info "   → Settings > Apps > Termux:X11 > Display over other apps → Allow"
  info "   → Settings > Battery > Battery optimization → [Termux:X11] → Don't optimize"
  echo ""
  pecho "$PASTEL_PINK" "   For Termux:Widgets:"
  info "   → Long-press home screen → Widgets → Termux:Widget"
  info "   → Grant file access permissions when prompted"
  echo ""
  pecho "$PASTEL_YELLOW" "3. INSTALLATION PHASE (after downloads):"
  info "   - File manager will open automatically"
  info "   - Tap each .apk file to install"
  info "   - Follow Android's installation prompts"
  info "   - Configure permissions as described above"
  echo ""
  
  if [ "$NON_INTERACTIVE" != "1" ]; then
    pecho "$PASTEL_CYAN" "Press Enter when you understand these steps..."
    read -r || true
  fi
  
  echo ""
  info "Starting APK downloads (F-Droid versions)..."
  local failed=0
  ...
  # Install Termux:Widget (productivity shortcuts)
  ...
  echo ""
  draw_card "Ready for Installation" "Follow the permission instructions above"
  echo ""
  ...
  echo ""
  pecho "$PASTEL_YELLOW" "REMEMBER: After installing each APK:"
  info "1. Configure permissions as described above"
  info "2. Disable battery optimization for Termux:X11"
  info "3. Allow display over other apps for Termux:X11"
  echo ""
  
  # Pause for manual installation unless in non-interactive mode
  if [ "$NON_INTERACTIVE" != "1" ]; then
    info "Install all APK files by tapping them, configure permissions, then press Enter..."
    read -r || true
  else
...
# Step 10: User configuration and device detection
step_usercfg(){ 
  detect_phone
...
# Step 12: Simplified ADB Wireless Setup with single confirmation
step_adb(){
  ...
  if [ "$settings_opened" = true ]; then
    ok "Android Settings opened - follow the steps above"
  else
    info "Unable to open settings automatically - please open manually"
  fi
...
configure_bash_environment(){
  info "Setting up your custom Bash environment with personalized prompt..."
...
if [ -t 1 ] && command -v tput >/dev/null 2>&1 && [ "\$(tput colors 2>/dev/null || echo 0)" -ge 8 ]; then
  # Custom color palette for your personalized prompt
  PINK='\033[38;2;255;182;193m'         # Pink for username
  PURPLE='\033[38;2;221;160;221m'       # Purple for typed text
  CYAN='\033[38;2;175;238;238m'         # Cyan for path
  GREEN='\033[38;2;144;238;144m'        # Green for prompt symbol
  RESET='\033[0m'                       # Reset color
  
  # Your personalized command prompt with pink username and purple input
  if [ -n "\${PS1-}" ]; then
    # Format: username@phonetype:directory\$ (with purple input text)
    PS1="\[\$PINK\]${TERMUX_USERNAME:-\u}\[\$RESET\]@\[\$PINK\]${TERMUX_PHONETYPE:-\h}\[\$RESET\]:\[\$CYAN\]\w\[\$RESET\]\[\$GREEN\]\\\$\[\$RESET\] \[\$PURPLE\]"
    # Reset color after each command so output isn't purple
    PROMPT_COMMAND="echo -ne '\$RESET'"
  fi
else
  # Simple prompt for basic terminals
  if [ -n "\${PS1-}" ]; then
    PS1='${TERMUX_USERNAME:-\u}@${TERMUX_PHONETYPE:-\h}:\w\\\$ '
  fi
fi
...
if [ -n "\${PS1-}" ]; then
    ...
    echo -e "\033[38;2;255;182;193mTermux User: ${TERMUX_USERNAME:-unknown}\033[0m"
    echo -e "\033[38;2;255;182;193mPhone Type: ${TERMUX_PHONETYPE:-unknown}\033[0m"
...
export CAD_PHONE_TYPE="${TERMUX_PHONETYPE:-unknown}"

# Personalized aliases based on your setup
alias whoami_cad='echo "Termux User: \$CAD_TERMUX_USER@\$CAD_PHONE_TYPE, Git: \$CAD_GIT_USER <\$CAD_GIT_EMAIL>"'
...
change_username_color() {
  ...
  case "\$color" in
    ...
    *) echo "Available colors: pink, cyan, green, yellow, purple, blue"; return 1 ;;
  esac
  
  # Update the PINK color variable in .bashrc
  if command -v sed >/dev/null 2>&1; then
    sed -i "s/PINK=.*/PINK='\$new_color'/" "\$bashrc" 2>/dev/null
...
PERSONAL_CONFIG_EOF
  ...
  ok "Your personalized Bash environment is ready with:"
  info "  Pink username: ${TERMUX_USERNAME:-user}"
  info "  Phone type: ${TERMUX_PHONETYPE:-unknown}"
  info "  Purple typed text"
  info "  Custom welcome message"
  warn "Restart your terminal or run 'source ~/.bashrc' to see the new prompt"
}
